    ...
    <div class="flex w-full py-2 border-slate-900 border-t" v-if="username !== undefined && userPermissions !== undefined">
        ...
        <div class="w-2/3 flex justify-evenly">
            <div v-if="userPermissions.includes('GET') || manage" @click="" class="p-4 hover:text-violet-500 duration-300 relative">
                ...
                @click="$emit('grant', 'GET')"
                    class="absolute right-1 top-1 text-sm text-rose-500 hover:text-rose-600 duration-300">
                    x
                </span>
                <span v-if="manage && !userPermissions.includes('GET')" @click="$emit('remove', 'GET')"
                    class="absolute right-1 top-1 text-sm text-blue-300 hover:text-green-600 duration-300">
                    +
                </span>
            </div>
            <div v-if="userPermissions.includes('PUT')" @click="" class="p-4 hover:text-violet-500 duration-300 relative">
                EDIT
                <span v-if="manage" class="absolute right-1 top-1 text-sm text-rose-500 hover:text-rose-600 duration-300">x</span>
            </div>
            <div v-if="userPermissions.includes('DELETE')" @click="" class="p-4 hover:text-violet-500 duration-300 relative">
                DELETE
                <span v-if="manage" class="absolute right-1 top-1 text-sm text-rose-500 hover:text-rose-600 duration-300">x</span>
            </div>
        </div>
